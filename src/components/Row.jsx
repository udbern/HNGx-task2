          ...
          <div className="grid justify-center items-center grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 h-full md:gap-5 px-5">
            {movies.map((item) => (
              <Link data-testid="movie-card"
                to={`/movie/${item.id}`} 
                key={item.id}
                className="border h-[230px] md:h-[490px] shadow-3xl"
              >
                <div className="relative w-full group h-36 md:h-[370px]">
                  ...
                  />
                  {like ? (
                    <AiOutlineHeart className="hidden group-hover:block absolute border top-3 right-3 cursor-pointer text-3xl text-white" />
                  ) : (
                    <AiFillHeart className="text-3xl hidden group-hover:block cursor-pointer text-gray-100 absolute border top-3 right-3 h-5 w-5 md:h-8 md:w-8 p-1 bg-gray-400 rounded-full" />
                  )}
                </div>
                <h1 data-testid="movie-title" className="p-1 capitalize font-bold text-xs md:text-xl">
                  {item?.title}
                </h1>
